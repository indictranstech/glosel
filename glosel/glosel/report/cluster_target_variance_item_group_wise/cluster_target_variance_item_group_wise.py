...
def execute(filters=None):
	...
	data = []
	print "territory_item_group_dict.items()",territory_item_group_dict.items()
	for territory, territory_items in territory_item_group_dict.items():
		print "\n\nterritory",territory,territory_items
		for item_group, monthwise_data in territory_items.items():
			...
			data.append(row)
	for i in data:
		i.insert(0,frappe.db.get_value("Post Code Cluster",{"territory":i[0]},"name"))

	print "\n\ndata",data,"end data"
	print sorted(data, key=lambda x: (x[0], x[1])),"/ndata\n\n"
...
def get_columns(filters):
	...
	columns = [_("Post Code Cluster") + ":Data:120",_("Territory") + ":Link/Territory:120", _("Item Group") + ":Link/Item Group:120"]
